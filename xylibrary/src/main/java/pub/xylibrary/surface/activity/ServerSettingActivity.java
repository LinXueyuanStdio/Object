...
package pub.xylibrary.surface.activity;
...
import pub.xylibrary.R;
import pub.xylibrary.bone.base.BaseActivity;
import pub.xylibrary.bone.interfaces.OnBottomDragListener;
import pub.xylibrary.bone.order.Settings;
import pub.xylibrary.bone.util.DataKeeperUtil;
...
public class ServerSettingActivity extends BaseActivity implements OnClickListener, OnBottomDragListener {
	...
	public void initData() {//必须调用
		...
		tvServerSettingNormalName.setText(SERVER_NAMES[0]
				+ (Settings.isOnTestMode == false ? "[正在使用]" : ""));
		tvServerSettingTestName.setText(SERVER_NAMES[1]
				+ (Settings.isOnTestMode ? "[正在使用]" : ""));
	}
	...
	private void saveAndExit(boolean isTest) {
		if (StringUtil.isNotEmpty(sharedPreferencesPath, true)
			...
			Settings.putBoolean(Settings.KEY_IS_ON_TEST_MODE, isTest);
			DataKeeperUtil.save(sharedPreferencesPath, pathMode, isTest ? testKey : normalKey
					, StringUtil.getNoBlankString(isTest ? etServerSettingTest : etServerSettingNormal));
			showShortToast("已保存并切换至" + SERVER_NAMES[Settings.isOnTestMode ? 1 : 0]
					+ "，请不要退出登录。重启后生效");
		}
	...
	public void onDragBottom(boolean rightToLeft) {
		if (rightToLeft) {
			etServerSettingNormal.setText(StringUtil.getTrimedString(Settings.URL_SERVER_ADDRESS_NORMAL_HTTP));
			etServerSettingTest.setText(StringUtil.getTrimedString(Settings.URL_SERVER_ADDRESS_TEST));
			return;
		}	
